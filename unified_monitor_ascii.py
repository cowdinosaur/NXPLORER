#!/usr/bin/env python3
"""
Unified Environmental Monitoring and Plant Health Scanner (ASCII Version)
Combines CO2/humidity sensing (SCD30), light reading (LDR), and photo classification.
Runs measurements at 10-second intervals.
Uses ASCII characters instead of emojis for better compatibility.
"""
...
try:
    ...
    print("[WARNING] SCD30 libraries not available. CO2/humidity sensing disabled.")
...
try:
    ...
    print("[WARNING] LDR libraries not available. Light sensing disabled.")
    LDR_AVAILABLE = False
...
class UnifiedMonitorASCII:
    ...
    def _load_ai_model(self):
        ...
        if not os.path.exists(model_path) or not os.path.exists(labels_path):
            print("[AI] [ERROR] Model files not found")
            return
    ...
    def log_data(self, timestamp, co2, temp, humidity, light_raw, light_volts, prediction, confidence):
        ...
        try:
            ...
            print(f"[LOG] [ERROR] Failed to log data: {e}")
    ...
    def run_continuous(self, num_cycles=None):
        ...
        print(f"\n[START] Starting unified monitoring...")
        print(f"[CONFIG] Interval: {self.interval} seconds")
        print(f"[CONFIG] Plant: {self.plant_type}")
        if num_cycles:
            print(f"[CONFIG] Cycles: {num_cycles}")
        print(f"[CONFIG] Camera: {'Enabled' if self.use_camera else 'Disabled'}")
        print(f"[CONFIG] Log file: {self.log_file}")
        print(f"[CONFIG] Verbose: {'Enabled' if self.verbose else 'Disabled (sensor messages only)'}")
        ...
        try:
            ...
            print(f"\n\n[STOP] Monitoring stopped by user after {cycle_count} cycles")
        except Exception as e:
            print(f"\n\n[FATAL] Monitoring stopped due to error: {e}")

        print(f"[SUMMARY] Total cycles completed: {cycle_count}")
        print(f"[SUMMARY] Data saved to: {self.log_file}")
...
def main():
    parser = argparse.ArgumentParser(description='Unified Environmental Monitoring and Plant Health Scanner (ASCII Version)')
